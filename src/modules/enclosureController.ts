...
class EnclosureController {
    ...
    //Boardからhashmapを作成
    public createHashmapFromSquare(board: Board): {[key: string]:Enclosure | null}{
        const sizeX: number = board.size.x;
        const sizeY: number = board.size.y;
        let hashmap: {[key: string]:Enclosure | null} = {};
    ...
    public createInitialEnclosure(board: Board): EnclosureController {
        const initialPoint: number = 4; //一旦仮
        const topLeftSquare: Square = board.squares[initialPoint][initialPoint]
        const topRightSquare: Square = board.squares[initialPoint + 1][initialPoint]
        const bottomLeftSquare: Square = board.squares[initialPoint][initialPoint + 1]
        const bottomRightSquare: Square = board.squares[initialPoint + 1][initialPoint + 1]
    ...
    //Squareから8方向検索
    public checkAllDirectionsFromSquare(board: Board, square: Square): EnclosureController {
    ...
    public linkEnclosureAll(enclosures: Enclosure[]): EnclosureController {
        ...
        for(let i: number = 1; i < enclosures.length; i++) {
